...
const CONTRACT_ALLOW_LIST_ADDRESS =
...
task("deployerAllowList:readRole", "Gets the network deployer allow list")
  ...
  .setAction(async (args, hre) => {
    const allowList = await hre.ethers.getContractAt(
      "IAllowList",
      CONTRACT_ALLOW_LIST_ADDRESS
    );
...
task(
  "txAllowList:revoke",
  "Removes the address from the transaction allow list"
)
  .addParam("address", "the address you want to revoke all permission")
  .setAction(async (args, hre) => {
    const allowList = await hre.ethers.getContractAt(
...
task("minter:mint", "Mints native tokens")
  ...
  .setAction(async (args, hre) => {
    const minter = await hre.ethers.getContractAt(
      "INativeMinter",
      MINT_ADDRESS
    );
...
task("feeManager:get", "Gets the fee config").setAction(async (_, hre) => {
  const feeManager = await hre.ethers.getContractAt(
    "IFeeManager",
    FEE_MANAGER_ADDRESS
  );
...
task(
  "rewardManager:currentRewardAddress",
  "Gets the current configured rewarding address"
).setAction(async (_, hre) => {
  const rewardManager = await hre.ethers.getContractAt(
    "IRewardManager",
    REWARD_MANAGER_ADDDRESS
  );
...
task(
  "rewardManager:areFeeRecipientsAllowed",
  "Gets whether the fee recipients are allowed to receive rewards"
).setAction(async (_, hre) => {
  const rewardManager = await hre.ethers.getContractAt(
    "IRewardManager",
    REWARD_MANAGER_ADDDRESS
...
task(
  ...
  "Allows custom fee recipients to receive rewards"
).setAction(async (_, hre) => {
  const rewardManager = await hre.ethers.getContractAt(
    "IRewardManager",
    REWARD_MANAGER_ADDDRESS
...
task(
  ...
  "Disables all rewards, and starts burning fees."
).setAction(async (_, hre) => {
  const rewardManager = await hre.ethers.getContractAt(
    "IRewardManager",
    REWARD_MANAGER_ADDDRESS
...
task("warpMessenger:getBlockchainID", "Gets the blockchain ID").setAction(
  async (_, hre) => {
    const warp = await hre.ethers.getContractAt(
      "WarpMessenger",
      WARP_MESSENGER_ADDRESS
    );
    const result = await warp.getBlockchainID();
    console.log(result);
  }
);
